...
# ── Reply keyboard shown after login ─────────────────────────────────────────
...
def otp_copy_menu(otp_code: str) -> InlineKeyboardMarkup:
    """Show OTP with a copy button (uses CopyTextButton if available)."""
    b = InlineKeyboardBuilder()
    try:
        ...
        # Older aiogram – just show text button that does nothing harmful
        b.button(text=f"📋 OTP: {otp_code}", callback_data="noop")
    return b.as_markup()
...
def active_numbers_menu(activations: list) -> InlineKeyboardMarkup:
    """Show each number with its own cancel button + a cancel-all button."""
    b = InlineKeyboardBuilder()
    for act in activations:
        phone = act.get("phoneNumber", "?")
        aid   = str(act.get("activationId", ""))
        b.button(
            text=f"❌ +{phone}",
            callback_data=f"active_cancel_{aid}",
        )
    b.adjust(1)
    b.button(text="🗑 Cancel ALL (Background)", callback_data="cancel_all_active")
    return b.as_markup()
